...
public class Game extends BaseGame implements ConnectionDelegate {
  ...
  public void startReuglarGame() {
    ...
    System.out
        .println("starting level: " + GameState.getInstance().getSelectedLevelBackground().getName().split("_")[0]);
  ...
  public byte[] onWrite() {
    ...
    if (state.getPlayers() != null && state.getPlayers().size() > 1) {
      gameState =
          state.getPlayers().get(index1).getScore() + "'" + state.getPlayers().get(index2).getLife() + "'" + state
              .getPlayers().get(index2).getScore() + "'" + state.players.get(index2).getLife() + "'" + state.players
              .get(0).getCurrentWeaponName() + "'" + state.players.get(1).getCurrentWeaponName() + "'" + state
              .getPlayer2Nick() + "'" + state.getPlayer1Nick();
    }
  ...
  public void setLevel(String terrain, String backGround) {
    ...
    if ((terrainFile = new File(terrain)).exists() && (backGroundFile = new File(backGround)).exists()) {
      ...
      gameListener.ConnectionFailed(
          "Could not find level file. Terrain:" + terrainFile.getName() + " Background:" + backGroundFile.getName());
