...
public class GameOverlay {
  ...
  public void paintOverlay(Graphics2D g, int score1, int score2, int lives1, int lives2, Gun wep1, Gun wep2) {
    ...
    if (GameState.getInstance().isRunning()) {
      ...
      String
          statsOne =
          String.format("%s I %s I %s", GameState.getInstance().getPlayer1Nick(), playerOneScore,
                        playerOne.getCurrentWeaponName());
      String
          statsTwo =
          String.format("%s I %s I %s", playerTwo.getCurrentWeaponName(), playerTwoScore,
                        GameState.getInstance().getPlayer2Nick());
      ...
      g.fillRect(border + border + alignMent, border, (width / 2 - border * 2) - border,
                 (height - border * 2) - border);
      g.fillRect(width / 2 + border + alignMent, border, (width / 2 - border * 2) - border,
                 (height - border * 2) - border);
      ...
      for (int i = 0; i < playerOne.getLife(); i++) {
        g.drawImage(ResourceManager.getInstance().HEART.getRgbBufferedImage(),
                    GameState.getInstance().getWidth() / 2 - 20 - i * 14, 15, null);
      }
      for (int i = 0; i < playerTwo.getLife(); i++) {
        g.drawImage(ResourceManager.getInstance().HEART.getRgbBufferedImage(),
                    GameState.getInstance().getWidth() / 2 + 7 + i * 14, 15, null);
      }
    }
  }
